...
//#endif
        ...
        private bool useUnobstructedFastPath;
        ...
        public void LineOfSightPhase1()
        {
            ...
            // A zero layer mask cannot produce a physics hit. The regular path would still
            // schedule ViewAngle * RaycastResolution raycasts and several follow-up jobs, only
            // to collapse their all-miss result to the same three segment records assembled
            // below. Keep the full path available as soon as a layer is enabled.
            useUnobstructedFastPath = ObstacleMask.value == 0;
            if (useUnobstructedFastPath)
                return;
        ...
        /// <summary>
        /// Reproduces the data generated by an all-miss raycast pass without touching PhysX or
        /// the Job System. This is exact for an empty obstacle mask: every original ray returned
        /// RayDistance, and the sorting stage retained only the endpoints represented here.
        /// </summary>
        private void BuildUnobstructedSight()
        {
            float firstAngle = ((-GetEuler() + 360 + 90) % 360) - (ViewAngle / 2);

            AddUnobstructedPoint(firstAngle);

            // With one surviving all-miss endpoint the original phase 2 inserts this synthetic
            // midpoint for both cones and complete circles. Its geometry fields are zeroed there;
            // only angle/radius/hit reach the shader, but reproduce all fields for debug parity.
            AddViewPoint(false, RayDistance, firstAngle + ViewAngle * 0.5f, 0f,
                float2.zero, float2.zero, float2.zero);

            // A complete circle duplicates its first point and ApplyData advances that final
            // angle by 360 degrees. An open cone retains its independently calculated endpoint.
            AddUnobstructedPoint(CircleIsComplete ? firstAngle : firstAngle + ViewAngle);
        }

        private void AddUnobstructedPoint(float angle)
        {
            Vector3 direction3D = DirFromAngle(angle, true);
            float2 direction;
            float2 origin;
            ...
            switch (FogOfWarWorld.instance.gamePlane)
            {
                case FogOfWarWorld.GamePlane.XY:
                    direction = new float2(direction3D.x, direction3D.y);
                    origin = new float2(EyePosition.x, EyePosition.y);
                    break;
                case FogOfWarWorld.GamePlane.ZY:
                    direction = new float2(direction3D.z, direction3D.y);
                    origin = new float2(EyePosition.z, EyePosition.y);
                    break;
                default:
                    direction = new float2(direction3D.x, direction3D.z);
                    origin = new float2(EyePosition.x, EyePosition.z);
                    break;
            }

            direction = math.normalize(direction);
            AddViewPoint(false, RayDistance, angle, 0f, -direction,
                origin + direction * RayDistance, direction);
        }
